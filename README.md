...
- 税率を直接「10%」と書くのではなく、変数 `$tax_rate` を使って表示することで、後から税率が変わっても1か所の修正で対応できることを学んだ。
- `<strong>` タグを使うことで、重要な情報を太字で強調できることを学んだ。
- practiceには自分で考えたコードを残し、sampleには教材どおりのコードを書くことで、自分の成長を振り返りやすくなることを意識した。
